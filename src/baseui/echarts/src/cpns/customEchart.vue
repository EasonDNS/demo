<template>
  <div class="custom-echart">
    <base-echart :options="options" ref="baseEchartRef"></base-echart>
  </div>
</template>
<script lang="ts">
...
export default defineComponent({
  ...
  setup(props) {
    const baseEchartRef = ref<InstanceType<typeof baseEchart>>()
    const anData = { ...props.anData }
    const data = [
      { value: 800, name: 'A' },
      { value: 635, name: 'B' },
      { value: 580, name: 'C' },
    ...
    setInterval(function () {
      ...
      baseEchartRef.value?.set(currentOption)
    }, 20000)

    // ---------------------------

    const options = pieOption
    return {
      baseEchartRef,
      options
    }
